...
export default {
    PacketCapture: (_interface: string, _callback: (_parsedPacket: {
        ...
        try {
            const _options = ['-i', _interface, '-l', '-n', '-vv']
            const _tcpdump = child.spawn('tcpdump', _options)
            ...
            let _packetCache = ''
            _tcpdump.on('close', () => { console.log('err') })
            _tcpdump.stdout.on('data', (_bufferData: Buffer) => {
    ...
    getFlows: async (
        ...
        _callback: (_flows: Array<{
            ...
            bps: number,
            specificFlow: {
                [ flow in 'INBOUND' | 'OUTBOUND' | 'UNKNOWN' ]: {
                    pps: number,
                    bps: number
                }
            }
        ...
        function _processFlow (
            ...
            if(_flowIdx == -1) {
                _flows.push({
                    ...
                    pps: 1,
                    specificFlow: {
                        INBOUND: { pps: _flow == 'INBOUND' ? 1 : 0, bps: _flow == 'INBOUND' ? _packet.size : 0 },
                        OUTBOUND: { pps: _flow == 'OUTBOUND' ? 1 : 0, bps: _flow == 'OUTBOUND' ? _packet.size : 0 },
                        UNKNOWN: { pps: _flow == 'UNKNOWN' ? 1 : 0, bps: _flow == 'UNKNOWN' ? _packet.size : 0 }
                    }
                })
            } else {
                _flows[_flowIdx].bps += _packet.size,
                _flows[_flowIdx].pps ++
                _flows[_flowIdx].specificFlow[_flow].bps += _packet.size,
                _flows[_flowIdx].specificFlow[_flow].pps ++
            }
        }
        const _pcaketCapture = monitorPlugin.PacketCapture(_interface, (_packet) => {
            _processFlow(_packet.flow, _packet)
            for(const _subpacket of _packet.subpackets) {
                _processFlow(_packet.flow, _subpacket)
            }
        })
        setInterval(() => {
